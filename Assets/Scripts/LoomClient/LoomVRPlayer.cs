...
public class LoomVRPlayer : MonoBehaviour
{
	...
	//-------------------------------------------------
	private static LoomVRPlayer _instance;
	public static LoomVRPlayer instance
	{
		get
		{
			if (_instance == null)
			{
				_instance = FindObjectOfType<LoomVRPlayer>();
			}
			return _instance;
		}
	}
	...
	private void ActivateRig(GameObject rig)
	{
		...
		if (pm)
        {
			PlayerManager modelPM = pm.hmd.GetComponentInChildren<PlayerManager>();
			pm.hmd = hmdTransform.gameObject;

			if (modelPM)
            {
				modelPM.transform.parent = pm.hmd.transform;
				modelPM.transform.localPosition = Vector3.zero;
				modelPM.transform.localRotation = Quaternion.identity;
			}
			

			if (rig == rigSteamVR)
            {
				pm.lefthand = leftHand.gameObject;
				pm.righthand = rightHand.gameObject;

				if (modelPM)
				{
					modelPM.lefthand.transform.parent = pm.lefthand.transform;
					modelPM.lefthand.transform.localPosition = Vector3.zero;
					modelPM.lefthand.transform.localRotation = Quaternion.identity;
					modelPM.righthand.transform.parent = pm.righthand.transform;
					modelPM.righthand.transform.localPosition = Vector3.zero;
					modelPM.righthand.transform.localRotation = Quaternion.identity;
				}
			} 
			else if (rig == rig2DFallback)
			{
				pm.lefthand = rig2DFallback.GetComponentInChildren<Hand>().gameObject;
				pm.righthand = pm.lefthand;
